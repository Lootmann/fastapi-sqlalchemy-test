...
class TestGetUser:
    ...
    def test_get_user_which_doesnt_exist(self, client, user_fixture):
        ...
        resp = client.get(f"/users/{user.id + 100}")
...
class TestGetPostByUser:
    ...
    def test_get_post_by_user_id_and_post_id(self, client, user_fixture):
        ...
        resp = client.get(f"/users/{user.id}/posts/{post_id}")
        assert resp.status_code == status.HTTP_200_OK

        # wrong post_id
        resp = client.get(f"/users/{user.id}/posts/{post_id + 1}")
        assert resp.status_code == status.HTTP_404_NOT_FOUND

        # wrong user_id
        resp = client.get(f"/users/{user.id+ 1}/posts/{post_id}")
        assert resp.status_code == status.HTTP_404_NOT_FOUND
...
class TestGetCommentByUser:
    def test_get_comments_by_user_id(self, client, user_fixture):
        ...
        resp = client.get(f"/users/{user.id}/comments")
        assert resp.status_code == status.HTTP_200_OK
    ...
    def test_get_comment_by_user_id(self, client, user_fixture):
        ...
        resp = client.get(f"/users/{user.id}/comments/{comment_id}")
        assert resp.status_code == status.HTTP_200_OK
        assert resp.json()["comment"] == "now testing..."

        # wrong comment_id
        resp = client.get(f"/users/{user.id}/comments/{comment_id + 1}")
        assert resp.status_code == status.HTTP_404_NOT_FOUND

        # wrong user_id
        resp = client.get(f"/users/{user.id + 1}/comments/{comment_id}")
        assert resp.status_code == status.HTTP_404_NOT_FOUND
...
class TestUpdateUser:
    ...
    def test_update_user_which_doesnt_exists(self, client, user_fixture):
        user: user_schema.UserCreateResponse = user_fixture
        resp = client.patch(f"/users/{user.id + 100}", json={"name": "updated"})
        assert resp.status_code == status.HTTP_404_NOT_FOUND
...
class TestDeleteUser:
    ...
    def test_delete_which_user_doesnt_exist(self, client, user_fixture):
        user: user_schema.UserCreateResponse = user_fixture
        resp = client.delete(f"/users/{user.id + 100}")
        assert resp.status_code == status.HTTP_404_NOT_FOUND
